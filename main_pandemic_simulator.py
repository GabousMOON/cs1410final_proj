...
class windows(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        style.configure("run.TButton", font=("Orbitron", 20, "bold"))
...
class OptionsPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = ttk.Button(nav_bar, text="Options", state=DISABLED, style="nav.TButton")
        label.grid(
            row=0,
            column=0,
            padx=0,
        ...
        switch_to_table_button.grid(row=0, column=2, padx=0, pady=0, sticky="nsew")

        # People options Frame
        self.people_options_frame = tk.Frame(self, bg="light blue")

        self.people_options_frame.pack(
            side="left", fill="both", expand=True, anchor="sw"
        )
        ...
        self.people_options_label = ttk.Label(
            self.people_options_frame,
            text="People Options",
            anchor="center",
            style="options_headers.TLabel",
        )
        ...
        self.social_factor_label.grid(
            row=4, column=0, padx=(10, 0), pady=10, sticky="w"
        )
        self.social_factor_entry = ttk.Entry(
            self.people_options_frame, textvariable=self.social_factor_var
        )
        self.social_factor_entry.grid(
            row=4, column=1, padx=(0, 10), pady=10, sticky="e"
        )
        ...
        self.incubation_len_label = ttk.Label(
            self.patho_options_frame,
            text="Incubation Length: ",
            style="options_labels.TLabel",
        )
        self.incubation_len_label.grid(
            row=5, column=0, sticky="w", padx=(10, 0), pady=10
        )
        self.incubation_len_entry = ttk.Entry(
            self.patho_options_frame, textvariable=self.incubation_len_var
        )
        self.incubation_len_entry.grid(
            row=5, column=1, sticky="e", padx=(0, 10), pady=10
        )
        ...
        self.symptom_infect_label = ttk.Label(
            self.patho_options_frame,
            text="Symptomatic Power: ",
            style="options_labels.TLabel",
        )
        self.symptom_infect_label.grid(
            row=6, column=0, sticky="w", padx=(10, 0), pady=10
        )
        self.symptom_infect_entry = ttk.Entry(
            self.patho_options_frame, textvariable=self.symptom_infect_var
        )
        self.symptom_infect_entry.grid(
            row=6, column=1, sticky="e", padx=(0, 10), pady=10
        )
        ...
        self.asymptom_infect_label = ttk.Label(
            self.patho_options_frame,
            text="Asymptomatic Power: ",
            style="options_labels.TLabel",
        )
        self.asymptom_infect_label.grid(
            row=7, column=0, sticky="w", padx=(10, 0), pady=10
        )
        self.asymptom_infect_entry = ttk.Entry(
            self.patho_options_frame, textvariable=self.asymptom_infect_var
        )
        self.asymptom_infect_entry.grid(
            row=7, column=1, sticky="e", padx=(0, 10), pady=10
        )
...
Simulation goes until everyone has immunity.""",
            style="grid.TLabel",
        )
    ...
    def generate_population(self):
        healthy_people = (
            int(self.health_count_entry.get())
            if self.health_count_entry.get().isnumeric()
            else 0
        )
        asymp_people = (
            int(self.asymp_count_entry.get())
            if self.asymp_count_entry.get().isnumeric()
            else 0
        )
        symp_people = (
            int(self.symp_count_entry.get())
            if self.symp_count_entry.get().isnumeric()
            else 0
        )
    ...
    def generate_virus(self):
        ...
        symptomatic_infectability = (
            int(self.symptom_infect_entry.get())
            if self.symptom_infect_entry.get().isnumeric()
            else 0
        )
        asymptomati_infectability = (
            int(self.asymptom_infect_entry.get())
            if self.asymptom_infect_entry.get().isnumeric()
            else 0
        )
        longevity_factor = (
            int(self.path_len_entry.get())
            if self.path_len_entry.get().isnumeric()
            else 0
        )
...
class GraphPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = ttk.Button(nav_bar, text="Graph", state=DISABLED, style="nav.TButton")
        label.grid(row=0, column=1, padx=0, pady=0, sticky="nsew")
        ...
        switch_to_options_button.grid(row=0, column=0, padx=0, pady=0, sticky="nsew")
        switch_to_table_button = ttk.Button(
            nav_bar,
            text="Table",
            command=lambda: controller.show_frame(TablePage),
        ...
        plotbutton = ttk.Button(buttons_frame, text="Plot", command=self.plot)
    ...
    def plot(self):
        ...
        self.plot1.text(
            1,
            0.9,
            f"Simulation took {field.day_counter} days to finish",
            horizontalalignment="right",
            verticalalignment="center",
            transform=self.plot1.transAxes,
        )
    ...
    def rerun(self):
        ...
        field.population = []
        field.populate(
            healthy_people=health_initial,
            asymp_people=asymp_initial,
            symp_people=symp_initial,
        )
        field.add_pathogen(
            pathogen.type,
            pathogen.name,
            pathogen.damage_factor,
            pathogen.cure_defense,
            pathogen.longevity_factor,
            pathogen.incubation_len,
            pathogen.symptomatic_infectability,
            pathogen.asymptomatic_infectability,
            pathogen.surface_infect_factor,
        )
        while field.healthy_people_count != len(field.population):
            field.update()
        self.plot()
...
class TablePage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = ttk.Button(nav_bar, text="Table", state=DISABLED, style="nav.TButton")
        label.grid(row=0, column=2, padx=0, pady=0, sticky="nsew")
        ...
        switch_to_graph_button.grid(row=0, column=1, padx=0, pady=0, sticky="nsew")
    ...
    def show_table(self):
        self.table_text.configure(
            text=field.hist_pop_data.show().to_string(max_rows=24), style="grid.TLabel"
        )
