...
class App(ctk.CTk):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.ageLabel.grid(row=0, column=0, padx=20, pady=20, sticky="ew")
        ...
        self.occupation_label.grid(row=4, column=0, padx=20, pady=20, sticky="ew")

        # Occupation combo box
        self.occupation_option_menu = ctk.CTkOptionMenu(
            self, values=["student", "working professional"]
        )
        self.occupation_option_menu.grid(
            row=4, column=1, padx=20, pady=20, columnspan=2, sticky="ew"
        )

        # Generate Button
        self.generate_results_button = ctk.CTkButton(
            self, text="Generate Results", command=self.generateResults
        )
        self.generate_results_button.grid(
            row=5, column=1, columnspan=2, padx=20, pady=20
        )
    ...
    def createText(self):
        ...
        text += (
            f"{self.genderVar.get()} currently a {self.occupation_option_menu.get()}"
        )
